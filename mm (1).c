...
void* mm_malloc(size_t size) {
  ...
  //
  // find a block that is free with enough space to work with
  ptr_free_block = search_free_list(req_size);
 // if not enough space after first search of the block, request more space and then search for block again
  if(ptr_free_block == NULL){
	  request_more_space(req_size);
	  ptr_free_block = search_free_list(req_size);
  }
  // remove the free block after we find one, then get the tags and size of the block
  ...
  // Checking to see if the block is full, and if it isn't, split the block into two pieces
  if(block_size-MIN_BLOCK_SIZE >=req_size){
	     ptr_free_block -> size_and_tags =req_size |preceding_block_use_tag | TAG_USED;
	     ptr_next = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block,req_size);
	     ptr_next ->size_and_tags =(size_t)((block_size-req_size)|TAG_PRECEDING_USED);
	      //set the footer
	     split = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block,block_size-WORD_SIZE);
	    *split = ptr_next ->size_and_tags;
	    //putting the pointer into the free block
	    insert_free_block(ptr_next);
  }
  // if Block is full, move on to next block
   else{
	ptr_free_block -> size_and_tags = ptr_free_block ->size_and_tags | TAG_USED;
        ptr_next = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, block_size);
        ptr_next->size_and_tags=ptr_next->size_and_tags | TAG_PRECEDING_USED;
   }
